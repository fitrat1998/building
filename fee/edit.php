...
$sql = mysqli_query($link,"SELECT * FROM fee WHERE id = '$id'");
$res = mysqli_fetch_assoc($sql);

$pr_id = $res['project_id'];
$ob_id = $res['object_id'];
$wk_id = $res['worker_id'];

$sql1 = mysqli_query($link," SELECT * FROM projects WHERE  id = '$pr_id'");
$res1 = mysqli_fetch_assoc($sql1);

$sql2 = mysqli_query($link," SELECT * FROM objects WHERE  id = '$ob_id'");
$res2 = mysqli_fetch_assoc($sql2);

$sql3 = mysqli_query($link," SELECT * FROM workers WHERE  id = '$wk_id'");
$res3 = mysqli_fetch_assoc($sql3);

$_SESSION['res_id'] = $res['id'];
                         ...
                         <div class="card" >
                            <div class="card-body" >
                             <form method="POST" id="edit" >
                               <h3 class="text-center border-bottom m-b-20">Updating</h3>
                                <div class="row mb-3" >
                                     

                                    <div class="col-lg-3" id="selworker">
                                        <i class=" fas fa-hospital "></i>
                                       <label> Worker</label>
                                      <input type="texts" name="worker" class="form-control" placeholder="object" required  style="background-color:#F8F8F8 !important;" value="<?=$res3['name']?>">
                                    </div>

                                    <div class="col-lg-3" id="selworker">
                                        <i class=" fas fa-dollar-sign "></i>
                                       <label> Fee</label>
                                       <input type="text"  name="fee" class="form-control object" placeholder="fee" required style="background-color: #F8F8F8 !important;" value="<?=$res['fee']?>">
                                    </div>
                                </div>
                                    <input type="hidden" name="pr_id" value="<?=$res1['id']?>">
                                    <input type="hidden" name="ob_id" value="<?=$res2['id']?>">
                                    <input type="hidden" name="wk_id" value="<?=$res3['id']?>">
                                   <button type="submit" class="btn btn-success float-justify"><h4>Update</h4></button>
                                 </form>
                                </div>
                            </div>
                         </div>
