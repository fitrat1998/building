...
if (empty($end_time) && empty($start_time)) {
    $query = "SELECT * FROM `extra_object` WHERE object_name='$ide' AND flat ='0' AND floor <=> '0'";
} else {
    if (!empty($end_time) && empty($start_time)) {
        $query = "SELECT * FROM `extra_object` WHERE object_name='$ide'  AND flat = '0' AND floor <=> '0' AND created_date <='$end_time'";
    }

    elseif (empty($end_time) && !empty($start_time)) {
        $query = "SELECT * FROM `extra_object` WHERE object_name='$ide'  AND flat = '0' AND floor <=> '0' AND created_date >='$start_time'";
    }
    
    elseif(!empty($end_time) && !empty($start_time)){
        $query = "SELECT * FROM `extra_object` WHERE object_name='$ide'  AND flat = '0' AND floor <=> '0' AND created_date BETWEEN '$start_time' AND '$end_time'";
    }
}
